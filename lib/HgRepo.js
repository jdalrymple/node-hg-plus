...
module.exports = class HgRepo {
  ...
  gitify(options, done = undefined) {
    ...
    return Fs.removeAsync(Path.resolve(gitRepo, `${this.name}-git`))
      .then(() => Command.run(`init ${this.name}-git`, gitRepo, ['--bare'], 'git'))
      .then(() => Command.run('bookmarks hg', this.path))
      .then(() => Command.run('push', gitRepo)
      .then(() => Command.run('config', this.path, ['--bool', 'core.bare', 'false'],'git'))
      .catch((results) => {
