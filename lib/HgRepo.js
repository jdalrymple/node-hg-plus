...
module.exports = class HgRepo {
  constructor(
    options = { url: null, username: '', password: '', path: process.cwd() },
    pythonPath = 'python') {
  ...
  push(
    ...
    if (options.password || options.username) {
      url = `https://${options.username}:${options.password}@${options.url
        .split('@')
        .pop()}`;
    } else {
  ...
  update(
    options = { clean: false, check: false, revision: null, tool: null },
    done = undefined) {
    const optionArgs = [];
  ...
  gitify(
    ...
    return Command
      ...
      .then(() => Command.run(`git clone gitifyhg::${this.path} ${options.gitRepoPath}`))
      .asCallback(done);
  }
  ...
  merge(
    options = { force: false, revision: null, preview: false, tool: null },
    done = undefined) {
    const optionArgs = [];
