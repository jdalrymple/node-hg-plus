...
function ensureGitify(pythonPath) {
  return Command.run(`${pythonPath} -c 'import gitifyhg'`)
    .catch((output) => {
      ...
      return Command.run(`${pythonPath} setup.py install', '../utils/gitifyhg`);
    });
}
...
module.exports = class HgRepo {
   ...
   * @return {Promise<String>} - Console output
   */
  push(options = { force: false, revision: null, bookmark: null, branch: null, newBranch: false, ssh: null, insecure: false }, done = undefined) {
    const optionArgs = [];
   ...
   * @return {Promise<String>} - Console output
   */
  pull(options = { source: this.url, force: false, update: false, revision: null, bookmark: null, branch: null, newBranch: false, ssh: null, insecure: false }, done = undefined) {
    const optionArgs = [];
   ...
   * @params  {Function} [done] - Callback function
   */
  rename(source, destination, options = { after: false, force: false, include: null, exclude: null, dryRun: false }, done = undefined) {
    const optionArgs = [];
