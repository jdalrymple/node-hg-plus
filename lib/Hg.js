  ...
  return Promise.each(fromRepos, (fromRepo) => {
    ...
    return combinedRepo.pull({ source: fromRepo, force: true })
      ...
      .catch((errorInfo) => {
        ...
        if (!errorInfo.error.message.includes('merging with a working directory ancestor')) {
      ...
      .then(() => {
        ...
        return combinedRepo.merge()
          ...
          .catch((errorInfo) => {
            if (!errorInfo.error.message.includes('nothing to merge') && !errorInfo.error.message.includes('merging with a working directory ancestor')) {
              throw errorInfo.error;
            }
          });
      });
