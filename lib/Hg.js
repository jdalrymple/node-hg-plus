...
function cloneMultipleAndMerge(fromRepos, combinedRepo) {
  ...
  return Promise.each(fromRepos, (fromRepo) => {
    ...
    const subPath = Path.resolve(combinedRepo.path, name);
  
    return combinedRepo.pull({ source: fromRepo, force: true })
      .then(() => combinedRepo.update({ clean: true, revision: 'default' }))
      .then(() => Fs.mkdirsAsync(subPath))
      .then(() => Globby(['*', '!.hg']))
      .then((files) => {
        console.log("FILES")
        console.log(files)

        return Promise.all(files.map(file => Fs.moveAsync(file, Path.join(subPath, file)))))
      })
