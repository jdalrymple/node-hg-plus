...
import lex, { COMMENT, DSTRING_END, DSTRING_START, HERECOMMENT, HEREGEXP_END, HEREGEXP_START, IF, JS, LBRACKET, NEWLINE, OPERATOR, RBRACKET, RELATION, SSTRING_END, SSTRING_START, TDSTRING_END, TDSTRING_START, TSSTRING_END, TSSTRING_START } from 'coffee-lex';
...
function isStringAtPosition(start, end, context) {
  ...
  return startType === SSTRING_START && endType === SSTRING_END || startType === DSTRING_START && endType === DSTRING_END || startType === TSSTRING_START && endType === TSSTRING_END || startType === TDSTRING_START && endType === TDSTRING_END;
}
...
function convert(context) {
  ...
  function fixLocations(node) {
    ...
    switch (type(node)) {
      ...
      default:
        throw new Error('cannot fix location data for ' + type(node) + ' at ' + (node.locationData.first_line + 1 + ':' + (node.locationData.first_column + 1) + ': ') + inspect(node));
    }
  }
  ...
  function rangeOfBracketTokensForIndexNode(indexNode) {
    ...
    var range = context.sourceTokens.rangeOfMatchingTokensContainingTokenIndex(LBRACKET, RBRACKET, startTokenIndex);
    if (!range) {
      throw new Error('cannot find braces surrounding index at ' + (indexNode.locationData.first_line + 1 + ':' + indexNode.locationData.first_column + ': ') + ('' + inspect(indexNode)));
    }
    return range;
  }
    ...
    function makeNode(type$$1, loc) {
      ...
      if (result.range) {
        ...
        var lastTokenIndexOfNode = context.sourceTokens.lastIndexOfTokenMatchingPredicate(function (token) {
          return token.end <= result.range[1] && token.type !== NEWLINE && token.type !== COMMENT && token.type !== HERECOMMENT;
        });
