...
var SemicolonsStage = function () {
  ...
  createClass(SemicolonsStage, null, [{
    ...
    value: function run(content, filename) {
      ...
      var editor = new MagicString(content);
      var ast = parse(content, { sourceType: 'module', plugins: BABYLON_PLUGINS });
...
var NON_SEMANTIC_SOURCE_TOKEN_TYPES = [COMMENT, HERECOMMENT, NEWLINE];
...
var NodePatcher = function () {
  ...
  createClass(NodePatcher, [{
    ...
    value: function isSurroundedByParentheses() {
      ...
      var leftTokenType = LPAREN;
      var rightTokenType = RPAREN;
  ...
  createClass(ArrayInitialiserPatcher, [{
    ...
    value: function patchAsExpression() {
      ...
      this.members.forEach(function (member, i, members) {
        ...
        var needsComma = !isLast && !member.hasSourceTokenAfter(COMMA);
        member.patch();
        if (needsComma) {
  ...
  createClass(BinaryOpPatcher, [{
    ...
    value: function operatorTokenPredicate() {
      return function (token) {
        return token.type === OPERATOR || token.type === EXISTENCE;
      };
    }
  ...
  createClass(FunctionApplicationPatcher, [{
    ...
    value: function patchAsExpression() {
      ...
      args.forEach(function (arg, i) {
        ...
        var commaTokenIndex = arg.node.virtual ? null : _this2.indexOfSourceTokenAfterSourceTokenIndex(arg.outerEndTokenIndex, COMMA, isSemanticToken);
  ...
  createClass(FunctionPatcher, [{
    ...
    value: function patchAsExpression() {
      ...
      this.parameters.forEach(function (parameter, i) {
        ...
        var needsComma = !isLast && !parameter.hasSourceTokenAfter(COMMA);
        parameter.patch();
        if (needsComma) {
    ...
    value: function getArrowToken() {
      ...
      if (this.hasParamStart()) {
        var parenRange = this.getProgramSourceTokens().rangeOfMatchingTokensContainingTokenIndex(LPAREN, RPAREN, this.contentStartTokenIndex);
        var rparenIndex = parenRange[1].previous();
        arrowIndex = this.indexOfSourceTokenAfterSourceTokenIndex(rparenIndex, FUNCTION);
      }
          ...
          if (statement instanceof ReturnPatcher && !statement.expression) {
            ...
            if (statements.length > 1) {
              var startOfLineIndex = _this2.context.sourceTokens.lastIndexOfTokenMatchingPredicate(function (token) {
                return token.type === NEWLINE || token.type === SEMICOLON;
...
var MemberAccessOpPatcher = function (_NodePatcher) {
  ...
  createClass(MemberAccessOpPatcher, [{
    ...
    value: function getMemberOperatorSourceToken() {
      ...
      var dotIndex = this.indexOfSourceTokenAfterSourceTokenIndex(this.expression.outerEndTokenIndex, DOT);
  ...
  createClass(InterpolatedPatcher, [{
    ...
    value: function getInterpolationStartTokenAtIndex(index) {
      var interpolationStartIndex = this.indexOfSourceTokenBetweenPatchersMatching(this.quasis[index], this.expressions[index], function (token) {
        return token.type === INTERPOLATION_START;
      });
...
var ObjectInitialiserPatcher = function (_NodePatcher) {
  ...
  createClass(ObjectInitialiserPatcher, [{
    ...
    value: function isImplicitObject() {
      ...
      return tokens$$1.tokenAtIndex(indexOfFirstToken).type !== LBRACE;
    }
  ...
  createClass(ConditionalPatcher, [{
    ...
    value: function patchAlternateForStatement() {
      ...
      if (this.alternate) {
        ...
        var isElseIf = ifToken ? ifToken.type === IF : false;
        if (isElseIf) {
...
var ForOfPatcher = function (_ForPatcher) {
  ...
  createClass(ForOfPatcher, [{
    ...
    value: function removeOwnTokenIfExists() {
      if (this.node.isOwn) {
        var ownIndex = this.indexOfSourceTokenAfterSourceTokenIndex(this.contentStartTokenIndex, OWN);
...
var SeqOpPatcher = function (_BinaryOpPatcher) {
  ...
  createClass(SeqOpPatcher, [{
    ...
    value: function operatorTokenPredicate() {
      return function (token) {
        return token.type === SEMICOLON || token.type === NEWLINE;
      };
    }
  }]);
  return SeqOpPatcher;
...
var WhilePatcher = function (_LoopPatcher) {
  ...
  createClass(WhilePatcher, [{
    ...
    value: function patchAsStatement() {
      ...
      var isLoop = whileToken.type === LOOP;
  ...
  createClass(FunctionApplicationPatcher, [{
    ...
    value: function getFuncEnd() {
      if (this.node.type === 'SoakedFunctionApplication') {
        var questionMarkTokenIndex = this.indexOfSourceTokenAfterSourceTokenIndex(this.fn.outerEndTokenIndex, EXISTENCE);
...
function runWithPaths(paths) {
  ...
  function processDirectory(path$$1) {
    readdir(path$$1, function (err, children) {
      if (err) {
        errors.push(err);
      } else {
        pending.unshift.apply(pending, toConsumableArray(children.filter(function (child) {
          return extname(child) === '.coffee';
        }).map(function (child) {
          return join(path$$1, child);
        })));
      }
      processNext();
    });
  ...
  function processFile(path$$1) {
    var outputPath = join(dirname(path$$1), basename(path$$1, extname(path$$1))) + '.js';
    console.log(path$$1 + ' \u2192 ' + outputPath);
    runWithStream(path$$1, createReadStream(path$$1, { encoding: 'utf8' }), createWriteStream(outputPath, { encoding: 'utf8' }), function (err) {
      if (err) {
        errors.push(err);
      }
      processNext();
...
export { PatchError, convert$1 as convert, run$1 as run };
//# sourceMappingURL=decaffeinate.mjs.map
